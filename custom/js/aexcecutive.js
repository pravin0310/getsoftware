...
$("#deleteServiceModalBtn").unbind('click').bind('click', function() {
		...
		$("#submitServicedForm").unbind('submit').bind('submit', function() {
			...
			var serviceName = $("#dsname").val();
			
			
							
			if(serviceName == "") {
				$("#dsname").after('<p class="text-danger">  Service Name required </p>');
				$('#dsname').closest('.form-group').addClass('has-error');
			} else {
				$("#dname").closest('.form-group').addClass('has-success');
			} // /else
			...
			if( serviceName ) {
				...
				$.ajax({
		          ...
		          '</div>');
							...
							// remove the mesages
		          $(".alert-success").delay(50000).show(10, function() {
								$(this).delay(300000).hide(10, function() {
									$(this).remove();
								});
							}); // /.alert
		          ...
		          // reload the manage student table
					...
					//$("#submitServicedForm")[0].location.reload(); 
						...
						//	$("#submitServicedForm")[0].reset();
							...
							location.reload($(this).delay(50000));
						} // /if response.success
